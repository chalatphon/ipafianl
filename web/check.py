    ...
    try:
        ...
        with ConnectHandler(**device) as net_connect:
            ...
            try:
                ...
                if output and "Invalid input detected" not in output and "ambiguous command" not in output:
            ...
            try:
                # ส่งคำสั่งและตรวจสอบว่าผลลัพธ์ไม่มี error
                output = net_connect.send_command("show mac address-table", expect_string=r"#")
                if output and "Invalid input detected" not in output and "ambiguous command" not in output:
                    has_mac_table = True
            except Exception:
